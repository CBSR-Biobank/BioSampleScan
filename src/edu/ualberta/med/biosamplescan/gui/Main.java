...
public class Main extends org.eclipse.swt.widgets.Composite {
    ...
    private TableColumn[][] tableColumns =
        new TableColumn[MAXPLATES][MAXPLATES * 13];
    ...
    ProcessingDialog processingDialog =
        new ProcessingDialog(getShell(), SWT.NONE);
        ...
        try {
            ...
            pack();
            {
                ...
                for (int table = 0; table < MAXPLATES; table++) {
                    ...
                    tables[table] =
                        new Table(this, SWT.FULL_SELECTION | SWT.EMBEDDED
                    ...
                    for (int i = 0; i < 8; i++) {
                        tableItems[table][i] =
                            new TableItem(tables[table], SWT.NONE);
            ...
            for (int plate = 0; plate < MAXPLATES; plate++) {
                if (platesChanged[plate]) {
                    ...
                    if (configDialog.plates[plate][0]
                        ...
                        scanlibReturn =
                            scanlib.slCalibrateToPlate(configDialog.dpi,
                                plate + 1);
                        FIXSCANLIBINIWRITINGBUG();
                        switch (scanlibReturn) {
                            case (ScanLib.SC_SUCCESS):
                ...
                for (int c = 0; c < 12; c++) {
                    ...
                    if (append) {
                        if (tableItems[table - 1][r].getText(c + 1).isEmpty()) {
                            ...
                            row[c + 1] =
                                tableItems[table - 1][r].getText(c + 1);
                        }
                    }
                    else {
                        row[c + 1] = nulltoblankString(sc[r][c].getValue());
    ...
    private void menuSetModeWidgetSelected(SelectionEvent evt) {// TODO
        InputDialog dlg =
            new InputDialog(getShell(), "Plate Mode",
    ...
    private void menuScanImageFileWidgetSelected(SelectionEvent evt) {
        ...
        int scanlibReturn =
            scanlib.slScanImage(configDialog.dpi, 0, 0, 0, 0, saveLocation);
        FIXSCANLIBINIWRITINGBUG();
        switch (scanlibReturn) {
            case (ScanLib.SC_SUCCESS):
                break;
    ...
    private void menuPlateScanToFile(SelectionEvent evt) {
        ...
        if (evt.toString().indexOf("Plate ") > -1) {
            platenum =
                Integer.parseInt(evt.toString().substring(
        ...
        int scanlibReturn =
            scanlib.slScanPlate(configDialog.dpi, platenum, saveLocation);
        FIXSCANLIBINIWRITINGBUG();
        switch (scanlibReturn) {
            case (ScanLib.SC_SUCCESS):
                break;
